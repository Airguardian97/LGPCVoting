...
def generate_ballot(userTp,userlgu,display_controls=False):
    ...
    for position in positions:
                ...
                candidates_data = candidates_data + '<li>' + input_box + '<button type="button" class="btn btn-primary btn-sm btn-flat clist platform" data-fullname="'+candidate.fullname+'" data-bio="'+candidate.bio+'"><i class="fa fa-search"></i> Platform</button><img src="' + \
                    ...
                    if position.cat == "National":
                        ...
                        if userlgu == candidate.lgu_id:
                            ...
                            candidates_data = candidates_data + '<li>' + input_box + '<button type="button" class="btn btn-primary btn-sm btn-flat clist platform" data-fullname="'+candidate.fullname+'" data-bio="'+candidate.bio+'"><i class="fa fa-search"></i> Platform</button><img src="' + \
                                image+'" height="100px" width="100px" class="clist"><span class="cname clist">' + \
                                candidate.fullname +'</span></li>'#str(candidate.lgu_id)+ userlgu + userTp +'</span></li>'   
        ...
        output = output + f"""<div class="row">	<div class="col-xs-12"><div class="box box-solid" id="{position.id}">
             <div class="box-header with-border" style="background-color:#264d73">
            <h3 class="box-title" style="color:white;"><b>{name}</b></h3>"""

        if display_controls:
            output = output + f""" <div class="pull-right box-tools">
        <button type="button"  style="color:white;" class="btn btn-default btn-sm moveup" data-id="{position.id}" {up}><i class="fa fa-arrow-up"></i> </button>
        <button type="button"  style="color:white;" class="btn btn-default btn-sm movedown" data-id="{position.id}" {down}><i class="fa fa-arrow-down"></i></button>
        </div>"""
...
def fetch_ballot(request):
    ...
    output = generate_ballot(userTp,userlgu,display_controls=True,)
...
def show_ballot(request):
    ...
    ballot = generate_ballot(userTp,userlgu,display_controls=False)
    context = {
        'ballot': ballot
    }
